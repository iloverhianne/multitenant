  ...
  if (!isset($_GET['action'])) {
    ...
    try {
      ...
      $stmt = $db->prepare("SELECT b.*, 
                   ...
                   (SELECT COALESCE(v.plate_no, r.walkin_plate) FROM repair_jobs r LEFT JOIN vehicles v ON r.vehicle_id = v.vehicle_id WHERE r.bay_id = b.bay_id AND r.tenant_id = b.tenant_id AND r.status NOT IN ('COMPLETED', 'CANCELLED') ORDER BY r.created_at DESC LIMIT 1) as plate_no,
                   (SELECT m.full_name FROM repair_jobs r JOIN mechanics m ON r.mechanic_id = m.mechanic_id WHERE r.bay_id = b.bay_id AND r.tenant_id = b.tenant_id AND r.status NOT IN ('COMPLETED', 'CANCELLED') ORDER BY r.created_at DESC LIMIT 1) as mechanic_name
                   FROM service_bays b 
                   WHERE b.tenant_id = ? 
  ...
  if (isset($_GET['action'])) {
    ...
    try {
      ...
      if ($_GET['action'] === 'fetch_dashboard_jobs_diagnostic') {
        ...
        try {
          ...
          if ($currentRole === 'CASHIER' || $currentRole === 'STAFF') {
            // CASHIER: Needs to collect payment for ongoing or finished work
            $query .= " WHERE j.tenant_id = ? AND j.status IN ('IN_PROGRESS', 'COMPLETED')";
          } else {
            // ADMIN/MECHANIC: Needs to see upcoming and active work
      ...
      if ($_GET['action'] === 'fetch_settlement_history') {
        ...
        try {
          $query = "SELECT 
                    ...
                    LEFT JOIN services s ON j.service_id = s.service_id
                    LEFT JOIN customers c ON j.customer_id = c.customer_id
                    WHERE j.tenant_id = ? AND j.status IN ('COMPLETED', 'SETTLED', 'CANCELLED', 'REJECTED')
      ...
      if ($_GET['action'] === 'fetch_services') {
        $stmt = $db->prepare("SELECT * FROM services WHERE tenant_id = ? ORDER BY service_name ASC");
        $stmt->execute([$tenant_id]);
        echo json_encode($stmt->fetchAll(PDO::FETCH_ASSOC));
        exit;
      }
      ...
      if ($_GET['action'] === 'fetch_mechanic_history') {
        try {
          ...
          $stmt = $db->prepare("SELECT t.*, v.plate_no, v.make, v.model, j.status as current_status
      ...
      if ($_GET['action'] === 'get_job_details') {
        try {
          ...
          $stmt = $db->prepare("SELECT j.*, 
                                COALESCE(v.plate_no, j.walkin_plate) as plate_number, 
                                COALESCE(v.model, j.walkin_model, 'Walk-in') as vehicle_model, 
                                s.service_name,
      ...
      if ($_GET['action'] === 'fetch_job_parts') {
        try {
          ...
          $stmt = $db->prepare("SELECT rp.*, i.item_name FROM repair_parts rp JOIN inventory i ON rp.item_id = i.item_id WHERE rp.job_id = ? AND rp.tenant_id = ?");
          $stmt->execute([$jid, $tenant_id]);
          echo json_encode($stmt->fetchAll(PDO::FETCH_ASSOC));
        } catch (Exception $e) {
          echo json_encode([]);
      ...
      if ($_GET['action'] === 'fetch_available_resources') {
        try {
          ...
          // Consolidated resource fetching: Only show AVAILABLE mechanics or the currently assigned one
          $mechanicQuery = "SELECT mechanic_id, full_name, status FROM mechanics WHERE tenant_id = ? AND (status = 'AVAILABLE' OR mechanic_id = ?) ORDER BY full_name ASC";
      ...
      if ($_GET['action'] === 'add_payment' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        try {
          ...
          // DATABASE PATCHES (Run once if needed)
          try {
            $db->exec("ALTER TABLE payments MODIFY customer_id INT NULL");
          } catch (Exception $e) {
          }
          try {
            $db->exec("ALTER TABLE payments ADD COLUMN walkin_name VARCHAR(100) NULL AFTER customer_id");
          } catch (Exception $e) {
          }

          // MAINTENANCE: Migrate existing Walk-in customers to payments and cleanup
          try {
            // 1. Move name to payments.walkin_name where customer is a WALKIN
            $db->exec("UPDATE payments p 
                       JOIN customers c ON p.customer_id = c.customer_id 
                       SET p.walkin_name = c.full_name, p.customer_id = NULL 
                       WHERE c.mobile = 'WALKIN' AND p.tenant_id = $tenant_id");
            // 2. Delete the walkin customer records
            $db->exec("DELETE FROM customers WHERE mobile = 'WALKIN' AND tenant_id = $tenant_id");
          } catch (Exception $e) {
          }
          ...
          if ($customerId === 'WALKIN') {
            ...
            $finalCustomerId = null; // Don't create a customer record
          ...
          // Handle Inventory Deduction if parts were added manually to this payment
          $partsJson = $_POST['parts_json'] ?? '[]';
  ...
  <script>
        ...
        if (mS && bS) {
          ...
          fetch('tenant-dashboard.php?action=fetch_available_resources')
            .then(r => r.json()).then(data => {
              ...
              (data.mechanics || []).forEach(m => { mS.innerHTML += `<option value="${m.mechanic_id}">${m.full_name}</option>`; });
    ...
    window.filterPartCombobox = function (q) {
      ...
      list.innerHTML = filtered.map(p => {
        ...
        return `<div onclick="window.selectPartForJob(${p.item_id}, '${(p.item_name || "").replace(/'/g, "\\'")}', ${itemPrice}, ${p.quantity})" 
                     style="padding:12px 15px; cursor:pointer; border-bottom:1px solid rgba(255,255,255,0.05); transition:0.2s;"
                     onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='transparent'">
            <div style="display:flex; justify-content:space-between; align-items:center;">
              <div>
                <div style="font-weight:700; font-size:0.85rem; color:white;">${p.item_name}</div>
                <div style="font-size:0.7rem; color:var(--text-dim);">${p.brand || 'No Brand'} ??? ${p.quantity} in stock</div>
              </div>
        ...
        .then(r => r.text()).then(t => {
          try {
            ...
            let options = data.map(i => {
              const stock = parseInt(i.quantity || 0);
              return `<option value="${i.item_id}" data-price="${i.price}" data-stock="${stock}">${i.item_name} (???${parseFloat(i.price).toLocaleString()})</option>`;
            }).join('');
            row.innerHTML = `
    ...
    window.syncPaymentParts = function () {
      ...
      rows.forEach(row => {
        ...
        if (sel && sel.value) {
          const opt = sel.options[sel.selectedIndex], price = parseFloat(opt.getAttribute('data-price') || 0), qty = parseInt(qtyInput.value || 0);
          parts.push({ item_id: sel.value, quantity: qty });
          totalParts += price * qty;
        }
      });
      const jsonInput = document.getElementById('pay_parts_json');
      if (jsonInput) jsonInput.value = JSON.stringify(parts);
      const amtInput = document.getElementById('pay_amount');
      const payJobId = document.getElementById('pay_job_id');
      if (amtInput && payJobId && !payJobId.value) amtInput.value = totalParts;
    };
...
<body style="overflow-x:hidden;">
            ...
            foreach ($all_plans as $p):
              ...
              <div class="plan-card" data-plan-id="<?php echo $p['plan_id']; ?>"
                ...
                <div style="margin-bottom:25px;">
                  ...
                  <span class="plan-cycle-label" style="font-size:1rem; color:#64748b;"><?php echo ($user_cycle === 'yearly') ? '/yr' : '/mo'; ?></span>
                </div>
                <button class="upgrade-select-btn btn-action"
                  style="width:100%; padding:15px; border-radius:15px; font-weight:800; <?php echo $is_current ? 'opacity:0.5; pointer-events:none;' : ''; ?>" type="button"
        ...
        .then(r => r.json()).then(data => {
          ...
          body.innerHTML = data.map(s => {
            ...
            return `<tr>
              ...
              <td>
                <button class="btn-outline" onclick="window.editService(${s.service_id}, '${safeName}', '${safeDesc}', ${s.price}, ${s.master_id || 'null'})">Edit</button>
        ...
        .then(res => res.text()).then(text => {
          try {
            ...
            body.innerHTML = data.map(a => {
              ...
              // Accurate Date Formatting: Manual split to avoid timezone shifts
              let apptDate = 'No Date';
              if (a.appointment_date && a.appointment_date !== '0000-00-00') {
                try {
                  const parts = a.appointment_date.split('-');
                  if (parts.length === 3) {
                    const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
                    apptDate = `${months[parseInt(parts[1]) - 1]} ${parseInt(parts[2])}, ${parts[0]}`;
                  }
                } catch (e) { apptDate = a.appointment_date; }
              }
              ...
              if (a.appointment_time) {
                try {
                  ...
                  const h12 = hh % 12 || 12;
                  apptTime = `${h12}:${m} ${suffix}`;
                } catch (e) { apptTime = a.appointment_time; }
              }

              const reqMech = a.requested_mechanic_name || '<em style="color:var(--text-dim); font-size:0.75rem;">(Not Specified)</em>';
              const reqMechEscaped = a.requested_mechanic_name ? a.requested_mechanic_name.replace(/'/g, "\\'") : '';

              let actionHtml = '';
              if (isPending) {
                actionHtml = `
                <div style="display:flex; gap:8px;">
                  <button class="btn-action" style="flex:1; padding:8px; font-size:0.75rem; background:#10b981; color:white; border:none; border-radius:10px;" onclick="window.processAppointment(${a.appointment_id}, 'CONFIRMED', '${reqMechEscaped}')">
              ...
              <tr style="border-bottom:1px solid rgba(255,255,255,0.03);">
                <td>
                  <div style="font-weight:800; color:white; font-size:0.9rem;">${apptDate}</div>
                  <div style="font-size:0.75rem; color:var(--accent); font-weight:700;"><i class="far fa-clock"></i> ${apptTime}</div>
                </td>
            ...
            console.error("Bookings Fetch Error:", e, text);
            body.innerHTML = '<tr><td colspan="7" style="text-align:center; color:var(--danger); padding:2rem;">Data Error: Could not parse appointments.</td></tr>';
          }
        }).catch(err => {
          body.innerHTML = '<tr><td colspan="7" style="text-align:center; color:var(--danger); padding:2rem;">Network Error: Could not reach server.</td></tr>';
        });
    };
      ...
      <div class="nav-item" data-view="inventory" onclick="window.navToView('inventory')"
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; width:100%;">
          ...
          <span
            ...
            <i class="fas fa-exclamation-triangle" style="font-size:0.7rem;"></i> <?php echo $low_stock_count; ?>
          </span>
          <?php endif; ?>
        </div>
      </div>
      ...
      <div class="nav-item" data-view="staff" onclick="window.navToView('staff')"
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; width:100%;">
          ...
          <span
            ...
            <i class="fas fa-clock" style="font-size:0.7rem;"></i> <?php echo $pending_shift_requests_count; ?>
          </span>
          <?php endif; ?>
        </div>
      </div>
  ...
  <main class="main-content">
            ...
            <tbody id="shiftRequestsBody">
              ...
              <tr style="border-bottom:1px solid rgba(255,255,255,0.03);">
                <td style="padding:15px 20px;">
                  <div style="font-weight:700; color:#fff;"><?php echo htmlspecialchars($req['full_name']); ?></div>
                  <div style="font-size:0.75rem; color:var(--text-dim);">Mechanic ID:
                    #<?php echo $req['mechanic_id']; ?></div>
                </td>
                <td style="padding:15px 20px;">
                  <div style="color:var(--accent); font-weight:700; font-size:0.9rem;"><i class="far fa-clock"
                      style="margin-right:4px;"></i><?php echo date('h:i A', strtotime($req['requested_start'])); ?>
                    &ndash; <?php echo date('h:i A', strtotime($req['requested_end'])); ?></div>
                  ...
                  <div style="font-size:0.72rem; color:var(--text-dim); margin-top:3px;"><i class="fas fa-calendar-week"
                      style="color:var(--accent); margin-right:4px;"></i><?php echo implode(' &middot; ', array_map('trim', explode(',', $req['requested_days']))); ?>
                  </div>
                  <?php endif; ?>
                </td>
                <td style="padding:15px 20px;">
    ...
    <div id="view_settled_jobs" class="dashboard-view" style="display:none;">
      <div class="glass-panel" style="padding:2rem;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:2rem;">
          <h3><i class="fas fa-history" style="color:var(--accent); margin-right:10px;"></i> Settled Jobs History</h3>
          <button class="btn-outline" onclick="window.refreshSettledJobs()"><i class="fas fa-sync"></i> Refresh
            List</button>
        </div>
      ...
      <div class="glass-panel">
        <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom: 2rem;">
          <div>
            ...
            <p style="color:var(--text-dim);">Track physical bays to prevent double booking. <span
                ...
                <?php echo $plan_tier; ?> (Max <?php echo $bay_limit; ?> Bays)
        ...
        <div id="baysGrid"
          ...
          <div class="bay-card <?php echo !$is_avail ? 'clickable' : ''; ?>"
            ...
            <div style="position:absolute; top:-30px; right:-30px; width:120px; height:120px; background:<?php echo $is_avail ? 'var(--accent)' : '#ef4444'; ?>; opacity:0.05; filter:blur(40px);"></div>
            <div>
              <span class="badge <?php echo $is_avail ? 'badge-active' : 'badge-danger'; ?>" style="font-weight:800;">
                <?php echo strtoupper($bay['status']); ?>
              </span>
              ...
              <?php if (!$is_avail): ?>
                <div style="margin-top:1rem; padding:12px; background:rgba(255,255,255,0.03); border-radius:15px; border:1px solid rgba(255,255,255,0.05);">
                  <div style="font-size:0.75rem; color:#94a3b8; font-weight:700; text-transform:uppercase; margin-bottom:4px;">In Service</div>
                  <div style="font-weight:800; color:var(--accent); font-size:1.1rem;"><?php echo htmlspecialchars($bay['plate_no'] ?? 'N/A'); ?></div>
                  <div style="font-size:0.8rem; color:var(--text-dim); margin-top:5px;"><i class="fas fa-wrench" style="font-size:0.7rem;"></i> <?php echo htmlspecialchars($bay['mechanic_name'] ?? 'Unassigned'); ?></div>
                </div>
              <?php endif; ?>
            </div>

            <?php if ($is_avail): ?>
              <button class="btn-action" style="width:100%; margin-top:1.5rem; background:var(--accent); color:white; border:none; padding:1rem; border-radius:15px; font-weight:800; cursor:pointer;" 
              ...
              <td>
                <div style="display:flex; align-items:center; gap:12px; padding:0.5rem 0;">
                  <div
                    ...
                    <?php else: ?>
                    <span
                      style="font-size:0.85rem; font-weight:800; color:var(--accent);"><?php echo strtoupper(substr($staff['name'] ?? 'U', 0, 1)); ?></span>
                    <?php endif; ?>
                  </div>
                  <strong style="color:#fff;"><?php echo htmlspecialchars($staff['name']); ?></strong>
                </div>
              </td>
              <td>
                  ...
                  <div style="display:flex; align-items:center; gap:12px;">
                    ...
                    <?php if ($my_pending_shift_request): ?>
                      ...
                      <span class="badge"
                        ...
                        <i class="fas fa-<?php echo $icon; ?>"></i> <?php echo $s; ?>
                      </span>
                    <?php endif; ?>
                  </div>
                ...
                <?php if ($my_pending_shift_request && $my_pending_shift_request['status'] === 'PENDING'): ?>
                  <div
                    ...
                    <div>
                      ...
                      Requested New Hours:
                      <strong><?php echo date('h:i A', strtotime($my_pending_shift_request['requested_start'])); ?> -
                        <?php echo date('h:i A', strtotime($my_pending_shift_request['requested_end'])); ?></strong>
                    </div>
                  </div>
            ...
            <div style="display:flex; flex-wrap:wrap; gap:8px;">
              <?php foreach (['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] as $d): ?>
                <label
                  ...
                  <input type="checkbox" name="shift_days[]" value="<?php echo $d; ?>" <?php echo $d !== 'Sun' ? 'checked' : ''; ?> style="accent-color:var(--accent);"> <?php echo $d; ?>
                </label>
              <?php endforeach; ?>
            </div>
          </div>
            ...
            <?php foreach (['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] as $d): ?>
              <label
                ...
                <input type="checkbox" name="shift_days[]" value="<?php echo $d; ?>" class="edit-shift-day-cb"
                  style="accent-color:var(--accent); transform:scale(1.1);"> <?php echo $d; ?>
              </label>
            <?php endforeach; ?>
          </div>
        </div>
          ...
          <div style="display:flex; flex-wrap:wrap; gap:8px;">
            <?php foreach (['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] as $d): ?>
              <label
                ...
                onmouseover="this.style.background='rgba(255,255,255,0.1)'"
                onmouseout="this.style.background='rgba(255,255,255,0.05)'">
                <input type="checkbox" name="shift_days[]" value="<?php echo $d; ?>" <?php echo $d !== 'Sun' ? 'checked' : ''; ?> style="accent-color:var(--accent); transform:scale(1.1);"> <?php echo $d; ?>
              </label>
            <?php endforeach; ?>
          </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:0.8rem;">
          <div>
            ...
            <input type="number" name="amount" id="pay_amount" required step="0.01" placeholder="0.00"
              style="width:100%; background:rgba(0,0,0,0.25); border:1px solid rgba(255,255,255,0.1); color:white; padding:0.7rem; border-radius:10px; font-size:1rem; font-weight:800; outline:none;">
          </div>
          <div>
            <label
  ...
  <script>
      ...
      try {
        ...
        data.forEach(job => {
          ...
          html += `<tr>
                ...
                <td style="font-weight:800; color:var(--accent);">???${parseFloat(job.total_amount || 0).toLocaleString()}</td>
                <td><span class="badge ${statusClass}">${status}</span></td>
                <td><i class="fas fa-check-circle" style="color:#10b981"></i> Archived</td>
              </tr>`;
        });
    ...
    window.syncMasterService = function (el, formId) {
      ...
      if (opt.value) {
        ...
        const validate = () => {
          const val = parseFloat(priceInput.value);
          const min = parseFloat(info.min_price);
          const max = parseFloat(info.max_price);
          const submitBtn = form.querySelector('button[onclick*="Service"]');

          if (!isNaN(val) && (val < min || val > max)) {
            priceInput.style.borderColor = '#ef4444';
            priceInput.style.boxShadow = '0 0 10px rgba(239, 68, 68, 0.3)';
            hint.style.color = '#ef4444';
            hint.innerHTML = `<i class="fas fa-exclamation-triangle"></i> <b>WARNING:</b> Price must be between ???${min.toLocaleString()} and ???${max.toLocaleString()}`;
            if (submitBtn) {
            ...
            priceInput.style.borderColor = 'rgba(99, 102, 241, 0.5)';
            priceInput.style.boxShadow = 'none';
            hint.style.color = '#10b981';
            hint.innerHTML = `<i class="fas fa-check-circle"></i> Price Boundary: <b>???${min.toLocaleString()}</b> to <b>???${max.toLocaleString()}</b>`;
            if (submitBtn) {
              submitBtn.disabled = false;
              submitBtn.style.opacity = "1";
              submitBtn.style.cursor = "pointer";
    ...
    window.showPaymentPartsSelector = function () {
      fetch('tenant-dashboard.php?action=fetch_inventory')
        ...
        .then(data => {
          ...
          let options = data.map(i => {
            const stock = parseInt(i.quantity || 0);
            const stockColor = stock <= 0 ? '????' : (stock < 5 ? '??????' : '???');
            const stockText = stock <= 0 ? 'Out of Stock' : `${stock} available`;
            return `<option value="${i.item_id}" data-price="${i.price}" data-stock="${stock}">
              ${i.item_name} (???${parseFloat(i.price).toLocaleString()}) [${stockColor} ${stockText}]
            </option>`;
          }).join('');
          row.innerHTML = `
            <select style="flex:2; font-size:0.8rem; background:rgba(0,0,0,0.3); border:1px solid rgba(255,255,255,0.1); color:white; border-radius:8px; padding:6px;" onchange="window.syncPaymentParts()">
    ...
    window.syncPaymentParts = function () {
      ...
      const amtInput = document.getElementById('pay_amount');
      const jid = document.getElementById('pay_job_id').value;
      // Broken fragment removed
    }
  ...
  <script>
    ...
    window.navToView = function (viewId) {
      ...
      if (target) {
        ...
        // Refresh appropriate data if needed
        if (viewId === 'dashboard') {
        ...
        if (viewId === 'vehicles') {
          console.log("[NAV] Switching to Vehicles...");
          if (typeof window.refreshVehiclesList === 'function') window.refreshVehiclesList();
        }
            ...
            if (window.currentUserRole === 'CASHIER') {
              return `
                <tr class="hover-bright">
                  ...
                  <td><span class="badge ${j.status === 'IN_PROGRESS' ? 'badge-active' : 'badge-pending'}">${j.status}</span></td>
                  <td>
        ...
        .then(data => {
          ...
          body.innerHTML = data.map(j => `
            <tr>
              ...
              <td>${j.make} ${j.model}<br><small style="opacity:0.5;">${j.customer_name}</small></td>
              <td>${j.service_name}</td>
              <td>???${parseFloat(j.total_amount || 0).toLocaleString()}</td>
              <td><span class="badge badge-active">${j.status}</span></td>
              <td><button class="btn-outline" onclick="window.viewJobReceipt(${j.job_id})">Receipt</button></td>
            </tr>
          `).join('');
        });
    };
    ...
    window.openRecordPaymentModal = function (jobId, customerId, customerName, amount) {
      console.log("[PAYMENT] Opening for Job:", jobId, "Customer:", customerId);
      ...
      if (custSelect) {
        if (!customerId || customerId === 'null' || customerId === 'undefined') {
          ...
          window.toggleWalkInField('WALKIN');
          if (form.querySelector('input[name="walkin_name"]')) {
            form.querySelector('input[name="walkin_name"]').value = customerName || '';
          }
        } else {
          custSelect.value = customerId;
          window.toggleWalkInField(customerId);
  ...
  <script>
    ...
    (function () {
      ...
      if (typeof window.refreshShiftRequests === 'function') window.refreshShiftRequests();
      if (typeof window.refreshVehiclesList === 'function') window.refreshVehiclesList();
      if (typeof window.refreshAppointmentsList === 'function') window.refreshAppointmentsList();
      if (typeof window.refreshDashboardJobs === 'function') window.refreshDashboardJobs();
      if (typeof window.refreshServicesList === 'function') window.refreshServicesList();
      if (typeof window.refreshJobOrders === 'function') window.refreshJobOrders();
      if (typeof window.refreshBaysList === 'function') window.refreshBaysList();
      if (typeof window.refreshSettledJobs === 'function') window.refreshSettledJobs();
    })();

    window.onerror = function (msg, url, lineNo, columnNo, error) {
      const errDiv = document.createElement('div');
      errDiv.style.cssText = 'position:fixed; bottom:20px; right:20px; background:rgba(239,68,68,0.9); color:white; padding:15px; border-radius:10px; z-index:999999; font-size:0.8rem; max-width:300px; border:1px solid white;';
      errDiv.innerHTML = '<strong>System Error:</strong><br>' + msg + '<br><small>Line: ' + lineNo + '</small>';
